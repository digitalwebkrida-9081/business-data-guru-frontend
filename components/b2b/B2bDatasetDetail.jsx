...
import {
  getCountryData,
  generateSimulatedDistribution,
} from "../../data/countryStates";
import { countryCodes } from "../../utils/countryCodes";
import { MdKeyboardArrowDown } from "react-icons/md";
import PhoneInputField from "../ui/PhoneInputField";

const CountryMapSection = dynamic(() => import("./CountryMapSection"), {
  ssr: false,
});
...
const enrichWithMapData = (ds) => {
  ...
  if (
    ds.stateDistribution &&
    Object.keys(ds.stateDistribution).length > 0 &&
    ds.countryCode
  ) {
    return ds;
  }
  ...
  const rawPrev = ds.previousPrice
    ? String(ds.previousPrice).replace(/[^0-9.]/g, "")
    : "398";
  ...
  return {
    ...
    stateDistribution: generateSimulatedDistribution(
      countryInfo.states,
      totalRecords || 5000,
    ),
  };
...
const B2bDatasetDetail = ({ id, country, category, initialDataset = null }) => {
    ...
    try {
      ...
      if (dataset?.mergedData && country && category) {
        ...
        const response = await fetch(
          `${API_URL}/api/scraper/dataset/purchase`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      try {
        ...
        if (country && category) {
          try {
            ...
            if (filterCity)
              dataUrl += `&city=${encodeURIComponent(filterCity)}`;

            const [dataRes, catRes] = await Promise.all([
              fetch(dataUrl),
              fetch(
                `${API_URL}/api/merged/categories?country=${countryApiCode}&limit=10000`,
              ),
            ]);
            ...
            const totalRecords =
              filterState || filterCity
                ? dataTotal
                : catInfo?.records || dataTotal || 0;
            const rows = dataResult.success
              ? dataResult.data?.data || dataResult.data?.rows || []
              : [];
            ...
            const hasEmail =
              catInfo?.hasEmail ||
              columns.some((c) => c.toLowerCase().includes("email"));
            const hasPhone =
              catInfo?.hasPhone ||
              columns.some((c) => c.toLowerCase().includes("phone"));
            const hasWebsite =
              catInfo?.hasWebsite ||
              columns.some(
                (c) =>
            ...
            const hasLinkedin =
              catInfo?.hasLinkedin ||
              columns.some((c) => c.toLowerCase().includes("linkedin"));
            const hasFacebook =
              catInfo?.hasFacebook ||
              columns.some((c) => c.toLowerCase().includes("facebook"));
            const hasInstagram =
              catInfo?.hasInstagram ||
              columns.some((c) => c.toLowerCase().includes("instagram"));
            const hasTwitter =
              catInfo?.hasTwitter ||
              columns.some((c) => c.toLowerCase().includes("twitter"));
            const hasTiktok =
              catInfo?.hasTiktok ||
              columns.some((c) => c.toLowerCase().includes("tiktok"));
            const hasYoutube =
              catInfo?.hasYoutube ||
              columns.some((c) => c.toLowerCase().includes("youtube"));

            // Map rows to sample list format
            const sampleList = rows.slice(0, 10).map((row, idx) => {
              const nameCol =
                columns.find((c) =>
                  /^(name|business|company|title)/i.test(c),
                ) || columns[0];
              const cityCol = columns.find((c) => /^city/i.test(c));
              const stateCol = columns.find((c) =>
                /^(state|province|region)/i.test(c),
              );
              ...
              const reviewCol = columns.find((c) =>
                /^(review|total.?review)/i.test(c),
              );
            ...
            const mergedDataset = {
              ...
              linkedinCount: hasLinkedin ? Math.floor(totalRecords * 0.6) : 0,
              totalLinkedin: hasLinkedin ? Math.floor(totalRecords * 0.6) : 0,
              facebookCount: hasFacebook ? Math.floor(totalRecords * 0.65) : 0,
              totalFacebook: hasFacebook ? Math.floor(totalRecords * 0.65) : 0,
              instagramCount: hasInstagram ? Math.floor(totalRecords * 0.5) : 0,
              totalInstagram: hasInstagram ? Math.floor(totalRecords * 0.5) : 0,
              twitterCount: hasTwitter ? Math.floor(totalRecords * 0.3) : 0,
              totalTwitter: hasTwitter ? Math.floor(totalRecords * 0.3) : 0,
              tiktokCount: hasTiktok ? Math.floor(totalRecords * 0.2) : 0,
              totalTiktok: hasTiktok ? Math.floor(totalRecords * 0.2) : 0,
              youtubeCount: hasYoutube ? Math.floor(totalRecords * 0.25) : 0,
              totalYoutube: hasYoutube ? Math.floor(totalRecords * 0.25) : 0,
              price: catInfo?.price
                ? parseFloat(String(catInfo.price).replace(/[^0-9.]/g, ""))
                : 199,
              previousPrice: catInfo?.previousPrice
                ? parseFloat(
                    String(catInfo.previousPrice).replace(/[^0-9.]/g, ""),
                  )
                : 398,
        ...
        if (id) {
          try {
            const response = await fetch(
              `${API_URL}/api/scraper/dataset/${id}`,
            );
            if (response.ok) {
              const result = await response.json();
  ...
  const getConvertedPrice = (usdPrice) => {
    if (!userCurrency || !userCurrency.needsConversion || !userCurrency.rate)
      return null;
  ...
  return (
    <div className="bg-white min-h-screen font-sans text-slate-800">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="text-xs font-medium text-slate-400 mb-6 flex gap-2 items-center tracking-wide">
            ...
            <span className="text-slate-200">
              {dataset.category} in {dataset.location}
            </span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="lg:w-[60%]">
              <h1 className="text-2xl lg:text-4xl font-bold mb-6 leading-tight">
                ...
                {displayLabel
                  ? `in ${displayLabel}`
                  : dataset.location.split(",").slice(-2).join(",")}
              </h1>
              ...
              <p className="text-slate-300 text-sm md:text-base mb-8 leading-relaxed">
                ...
                {dataset.category} in {dataset.location} as of{" "}
                ...
                {dataset.category} which is 95.89% of all {dataset.category} in{" "}
                {dataset.location} are single-owner operations. The top three
                states with the most {dataset.category} are populated below.
                Average age of {dataset.category} in {dataset.location} is 3
                years and 10 months.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-8 mb-8 text-sm md:text-base">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-300 hover:text-white cursor-pointer hover:underline decoration-slate-500 underline-offset-4">
                      Email Addresses
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white text-[18px]">
                      {(
                        dataset.websiteCount ||
                        dataset.totalWebsites ||
                        0
                      ).toLocaleString()}
                    </span>
                    <span className="text-slate-300 hover:text-white cursor-pointer hover:underline decoration-slate-500 underline-offset-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white text-[18px]">
                      {(
                        dataset.linkedinCount ||
                        dataset.totalLinkedin ||
                        0
                      ).toLocaleString()}
                    </span>
                    <span className="text-slate-300 hover:text-white cursor-pointer hover:underline decoration-slate-500 underline-offset-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white text-[18px]">
                      {(
                        dataset.facebookCount ||
                        dataset.totalFacebook ||
                        0
                      ).toLocaleString()}
                    </span>
                    <span className="text-slate-300 hover:text-white cursor-pointer hover:underline decoration-slate-500 underline-offset-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white text-[18px]">
                      {(
                        dataset.instagramCount ||
                        dataset.totalInstagram ||
                        0
                      ).toLocaleString()}
                    </span>
                    <span className="text-slate-300 hover:text-white cursor-pointer hover:underline decoration-slate-500 underline-offset-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white text-[18px]">
                      {(
                        dataset.youtubeCount ||
                        dataset.totalYoutube ||
                        0
                      ).toLocaleString()}
                    </span>
                    <span className="text-slate-300 hover:text-white cursor-pointer hover:underline decoration-slate-500 underline-offset-4">
                  ...
                  <div className="flex items-end gap-3 mb-4">
                    {(() => {
                      ...
                      const discount =
                        priceVal && prevVal
                          ? Math.round(
                              ((parseFloat(prevVal) - parseFloat(priceVal)) /
                                parseFloat(prevVal)) *
                                100,
                            )
                          : 50;
                        ...
                        <>
                          ...
                          {convertedPrice && (
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <span className="text-slate-500 text-xs ml-1 bg-slate-800/50 px-2 py-0.5 rounded-full">
                                Live rate
                              </span>
                            </div>
                          )}
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-bold text-white">
              List of {dataset.category} in {dataset.location}
            </h2>
            <p className="text-slate-400 text-sm mt-2">
              A preview of the data fields included in the downloaded list.
            </p>
          </div>
              ...
              <tbody>
                {dataset.sampleList.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-slate-500 flex items-center gap-1 overflow-hidden text-ellipsis max-w-[200px]">
                      <MdLocationOn className="text-blue-500 shrink-0" />
                      {"Avaliable"}
                    </td>
                    ...
                    <td className="p-3 text-slate-500 flex items-center gap-1 overflow-hidden text-ellipsis max-w-[200px]">
                      <MdEmail className="text-blue-300 shrink-0" />{" "}
                      {"Available"}
                    </td>
                    <td className="p-3 text-slate-500">
                      <span className="flex items-center gap-1">
                        <MdPhone className="text-green-500 shrink-0" />{" "}
                        {"Available"}
                      </span>
                    </td>
      ...
      <div className="py-20 pb-0 bg-white">
        <div className="container mx-auto px-4">
          <div className="mb-12 text-left">
            <h2 className="text-2xl lg:text-2xl font-bold text-slate-900 mb-4">
              How DataSellerHub Data Services Helps Businesses
            </h2>
            <p className="text-slate-600 text-base md:text-lg leading-relaxed max-w-4xl">
              Explore the strategic advantages of our data scraping solutions
              for your business.
            </p>
          </div>
          ...
          <div className="text-left space-y-8 max-w-auto mb-16">
            <div>
              <h3 className="text-xl lg:text-2xl font-bold text-slate-900 mb-3 block border-none pb-0">
                Find the Best {dataset.category} in{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                | Verified Business Directory
              </h3>
              <p className="text-slate-600 text-sm md:text-base leading-relaxed">
                Are you searching for a reliable partner to handle your business
                needs? Identifying the right <strong>{dataset.category}</strong>{" "}
                in{" "}
                <strong>
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}
                </strong>{" "}
                is the first step toward successful product development and
                innovation. Our comprehensive database offers an exhaustive list
                of {dataset.category} currently operating across{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}
                , providing you with the exact contact points needed to move
                your project forward.
              </p>
            </div>

            <div>
              <h4 className="text-lg lg:text-xl font-bold text-slate-900 mb-3">
                Why Choose {dataset.category} in{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}
                ?
              </h4>
              <p className="text-slate-600 text-sm md:text-base leading-relaxed">
                The landscape for {dataset.category} is evolving rapidly. By
                focusing your search on {dataset.category} within{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}
                , you gain access to high-tier engineering talent and advanced
                additive manufacturing technologies. The{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                market is known for its strict quality controls, making{" "}
                {dataset.category} in this region some of the most dependable in
                the global market.
              </p>
            </div>
            ...
            <div>
              <h4 className="text-lg lg:text-xl font-bold text-slate-900 mb-3">
                Why DataSellerHub is Your Trusted Source for {dataset.category}{" "}
                Data
              </h4>
              <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-4">
                When it comes to sourcing a list of {dataset.category} in{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}
                , data accuracy is paramount. DataSellerHub stands out as the
                industry leader for several reasons:
              </p>
              <ul className="list-disc pl-6 space-y-2 text-slate-600 text-sm md:text-base leading-relaxed">
                <li>
                  <strong>Verified Authenticity:</strong> Unlike automated
                  scrapers, DataSellerHub ensures that every entry for{" "}
                  {dataset.category} in{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  is verified for accuracy, reducing bounce rates in your
                  outreach.
                </li>
                <li>
                  <strong>Deep Market Insight:</strong> We don&apos;t just
                  provide names; our{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  report offers deep insights into the operational scale of
                  these {dataset.category}.
                </li>
                <li>
                  <strong>Frequent Updates:</strong> The {dataset.category}{" "}
                  sector in{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  changes fast. DataSellerHub refreshes its database regularly
                  to reflect new company formations and contact changes.
                </li>
                <li>
                  <strong>Compliance Guaranteed:</strong> Our data collection
                  for {dataset.category} in{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  follows all local data protection regulations, ensuring your
                  marketing remains ethical and legal.
                </li>
              </ul>
            </div>
            ...
            <div>
              <h4 className="text-lg lg:text-xl font-bold text-slate-900 mb-3">
                Strategic Benefits of our{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                {dataset.category} Report
              </h4>
              <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-4">
                Acquiring our detailed report on {dataset.category} in{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}{" "}
              ...
              <ol className="list-decimal pl-6 space-y-2 text-slate-600 text-sm md:text-base leading-relaxed">
                <li>
                  <strong>Accelerate Procurement:</strong> Instantly find{" "}
                  {dataset.category} in{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  that match your specific material requirements (Resin, Metal,
                  Carbon Fiber).
                </li>
                <li>
                  <strong>Competitor Analysis:</strong> Map out where the top{" "}
                  {dataset.category} are located throughout{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  to identify market gaps.
                </li>
                <li>
                  <strong>Direct Outreach:</strong> Connect directly with
                  decision-makers at {dataset.category} across{" "}
                  {displayLabel ||
                    dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                  without gatekeepers.
                </li>
            ...
            <div>
              <h4 className="text-lg lg:text-xl font-bold text-slate-900 mb-3">
                Maximize Your ROI in the{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}{" "}
                Market
              </h4>
              <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-4">
                Investing in a high-quality list of {dataset.category} is an
                investment in your company&apos;s growth. Whether you are
                selling software to {dataset.category} or looking to outsource
                your production to{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}
                , our data provides the foundation for success.
              </p>
              <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-6">
                Stop wasting time with outdated spreadsheets. Trust
                DataSellerHub to provide the most authentic, up-to-date, and
                actionable list of {dataset.category} in{" "}
                {displayLabel ||
                  dataset.location.split(",").slice(-2).join(",").trim()}{" "}
              ...
              <div>
                <button
                  onClick={() => setIsModalOpen(true)}
                  className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm md:text-base font-bold transition uppercase tracking-wide cursor-pointer shadow-lg shadow-blue-500/20 inline-flex items-center gap-2"
                >
                  Get Instant Access to the {dataset.category} Report
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div className="bg-slate-50 p-8 rounded-xl border border-slate-100 hover:shadow-lg transition">
              ...
              <h3 className="font-bold text-lg mb-3">
                Re-target Your Email Campaigns
              </h3>
              <p className="text-sm text-slate-600 leading-relaxed">
                Our verified lists ensure high deliverability rates. Reach the
                right inbox at the right time. Segment your lists and
                personalize content for maximum engagement and conversion.
              </p>
            </div>
            {/* Card 2 */}
            <div className="bg-slate-50 p-8 rounded-xl border border-slate-100 hover:shadow-lg transition">
              ...
              <h3 className="font-bold text-lg mb-3">
                Master Your Cold Calling Impact
              </h3>
              <p className="text-sm text-slate-600 leading-relaxed">
                Avoid gatekeepers with direct dial numbers. Our data includes
                key decision-maker information, empowering your sales team to
            ...
            <div className="bg-slate-50 p-8 rounded-xl border border-slate-100 hover:shadow-lg transition">
              ...
              <h3 className="font-bold text-lg mb-3">
                Hack Your Direct Mail Campaigns
              </h3>
              <p className="text-sm text-slate-600 leading-relaxed">
                Physical mail is making a comeback. Use our precise address data
                to send physical offers, samples, or brochures directly to
                business locations in specific regions.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* --- COUNTRY MAP SECTION --- */}
      {dataset.stateDistribution &&
        Object.keys(dataset.stateDistribution).length > 0 && (
          <CountryMapSection
            category={dataset.category}
            location={dataset.location}
      ...
      <div className="py-16 bg-slate-50">
        <div className="container mx-auto px-4 border-b border-slate-200 pb-16">
          <div className="text-center mb-12">
            <h2 className="text-2xl font-bold">
              How many List of {dataset.category} are there in the{" "}
              {dataset.location}?
            </h2>
            <p className="text-slate-500 mt-2">
              There are a total of {dataset.totalRecords} {dataset.category}{" "}
              stores in {dataset.location} as of {dataset.lastUpdate}.
            </p>

            <div className="mt-8 flex justify-center gap-4">
              <button
                className="bg-blue-600 text-white px-6 py-2 rounded font-bold text-sm cursor-pointer"
                onClick={() => setIsModalOpen(true)}
              >
                Buy Full Dataset
              </button>
              <button
                className="bg-slate-800 text-white px-6 py-2 rounded font-bold text-sm flex items-center gap-2 cursor-pointer"
                onClick={() => setIsSampleModalOpen(true)}
              >
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 text-center">
              ...
              <div className="text-5xl font-black text-slate-800 mb-4">
                {(dataset.phones || dataset.totalPhones || 0).toLocaleString()}
              </div>
              <button className="text-xs uppercase font-bold border border-slate-300 px-4 py-1 rounded text-slate-500">
        ...
        <div className="container mx-auto px-4 py-16 pb-0 text-center">
          <h2 className="text-2xl font-bold mb-8">
            Unlock Monthly Insights for Market Analysis and Explosive Growth
          </h2>
          <div className="space-y-4 max-w-2xl mx-auto">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 text-sm font-medium text-slate-600">
              Leverage predictive analytics to forecast trend shifts in your
              locale.
            </div>
            <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 text-sm font-medium text-slate-600">
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-8 shadow-2xl relative animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="text-center mb-8">
              <h3 className="text-xl font-bold text-slate-900 mb-2">
                ...
                <span className="text-blue-600">{dataset.category}</span> in{" "}
                <span className="text-blue-600">{dataset.location}</span>
              </h3>
              <div className="flex flex-col items-center gap-1 mb-1">
                <span className="text-2xl font-extrabold text-slate-900">
                  ${String(dataset.price || 199).replace(/[^0-9.]/g, "")} USD
                </span>
                {getConvertedPrice(
                  String(dataset.price || 199).replace(/[^0-9.]/g, ""),
                ) && (
                    ...
                    <PayPalButtons
                      ...
                      createOrder={async (data, actions) => {
                        const API_URL =
                          ...
                          "https://stagservice.datasellerhub.com";
                        ...
                        const response = await fetch(
                          `${API_URL}/api/payment/create-paypal-order`,
                          {
                            method: "POST",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({
                      ...
                      onApprove={async (data, actions) => {
                        ...
                        try {
                          const response = await fetch(
                            `${API_URL}/api/payment/capture-paypal-order`,
                            {
                              method: "POST",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({
