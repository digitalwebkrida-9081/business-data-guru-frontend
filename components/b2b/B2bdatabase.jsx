    ...
    <table className="w-full text-left border-collapse">
      <thead>
        <tr className="bg-blue-50 text-blue-900 text-xs uppercase font-bold tracking-wider">
          <th className="p-4 border-b border-blue-100 whitespace-nowrap">
            Name
          </th>
          <th className="p-4 border-b border-blue-100 text-center whitespace-nowrap">
            Number of Records
          </th>
          <th className="p-4 border-b border-blue-100 text-center whitespace-nowrap">
            Emails
        ...
        {[...Array(5)].map((_, i) => (
          <tr key={i} className="hover:bg-slate-50 transition-colors">
            ...
            <td className="p-4 align-middle text-right">
              <div className="h-8 bg-slate-200 rounded w-48 ml-auto"></div>
            </td>
          </tr>
        ))}
      </tbody>
...
const B2bdatabase = ({ isSeoPage = false, initialFilters = {} }) => {
  ...
  const [selectedDatasetForSample, setSelectedDatasetForSample] =
    useState(null);
  ...
  const handleSampleDownload = async (e) => {
    ...
    setTimeout(async () => {
      ...
      XLSX.writeFile(
        wb,
        `${selectedDatasetForSample?.name?.replace(/ /g, "_") || "Sample"}_Leads.xlsx`,
      );
  ...
  const fetchStates = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/location/states?country=${encodeURIComponent(filters.country)}`,
      );
    ...
    if (filters.country && countries.length > 0) {
      ...
      const fetchMergedCategories = async () => {
        const countryObj = countries.find(
          (c) => (c.country_name || c.name) === filters.country,
        );
  ...
  useEffect(() => {
    const fetchCities = async () => {
      ...
      try {
        ...
        const API_URL = process.env.NEXT_PUBLIC_API_URL || "";
        const response = await fetch(
          `${API_URL}/api/location/cities/${stateObj.isoCode}?country=${encodeURIComponent(filters.country)}`,
        );
        const result = await response.json();
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const countryObj = countries.find(
        (c) => (c.country_name || c.name) === country,
      );
    ...
    <div className="bg-white min-h-screen font-sans text-slate-800">
        ...
        <div className="container mx-auto px-4 relative z-10 flex flex-col lg:flex-row items-center justify-between mb-8">
          <div className="lg:w-1/2 mb-10 lg:mb-0">
            ...
            <p className="text-slate-300 text-lg mb-8 max-w-lg">
              Access millions of businesses with verified emails, phone numbers,
              and key decision-maker data.
            </p>
          </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                      ...
                      <tbody className="divide-y divide-slate-100/50">
                        {datasets.map((item) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-5 text-slate-500 text-[14px] text-center align-middle whitespace-nowrap font-bold">
                              {item.records}
                            </td>
                            <td className="px-6 py-5 text-slate-500 text-[14px] text-center align-middle whitespace-nowrap">
                              {item.emails}
                            </td>
                            <td className="px-6 py-5 text-slate-500 text-[14px] text-center align-middle whitespace-nowrap">
                              {item.phones}
                ...
                <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-3">
                  <span className="text-sm text-slate-500">
                    {totalCategories > 0 &&
                      `Showing ${(currentPage - 1) * ITEMS_PER_PAGE + 1}–${Math.min(currentPage * ITEMS_PER_PAGE, totalCategories)} of ${totalCategories} datasets`}
                  </span>
                  {totalPages > 1 && (
      ...
      <div className="bg-white py-16">
        <div className="container mx-auto px-4">
          ...
          <div>
            <h3 className="text-[32px] font-bold text-center mb-8">
              Find <span className="text-blue-600">B2B Leads</span> Across
              Different Domains
            </h3>
              ...
              {countries.length > 0
                ? (() => {
                    ...
                    const rest = countries.filter(
                      (c) => !priorityNames.includes(getName(c)),
                    );
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="text-center max-w-3xl mx-auto">
            ...
            <p className="mt-4 text-gray-500 leading-relaxed text-[18px]">
              Maximize the potential of your marketing, sales, analytics, and
              operations with exclusive, high-quality leads. Reduce time spent
              on lead sourcing and focus on high-impact initiatives.
            </p>
          </div>
          ...
          <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-blue-600 text-white p-8 rounded-xl relative overflow-hidden group">
              <div className="relative z-10">
                <h4 className="text-lg font-semibold opacity-90 mb-2">
                  Close Faster
                </h4>
          ...
          <div className="mt-20 bg-[#F7FAFF] border border-blue-100 rounded-2xl p-10 flex flex-col lg:flex-row items-center gap-10 relative overflow-hidden">
            ...
            <div className="flex-1 max-w-xl">
              <h3 className="text-2xl font-semibold text-gray-900">
                ...
                <span className="text-[#3067FF]">Business Data Guru</span>
              </h3>

              <p className="mt-4 text-[#425466] leading-relaxed">
                Our web scraping services collect data and convert it into
                standardized CSV, Excel, and JSON formats. We deliver accurate,
                fast data scraping to meet the high-volume needs of enterprises.
                These samples illustrate the breadth of our web scraping
                capabilities across industries. We can extract data for any
                business sector.
              </p>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-8 shadow-2xl relative animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-slate-500 text-sm">
                Enter your details to download the sample list.
              </p>
            </div>
