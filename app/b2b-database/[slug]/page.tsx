...
export async function generateMetadata(
  ...
  if (!slug) {
    return {
      title: "Dataset Detail | Business Data Guru",
  ...
  if (match) {
    ...
    return {
      title: "Business Data Report | Business Data Guru",
      description: "View detailed business data and lead lists.",
    };
  }
...
export default async function DatasetDetailPage({ params }: Props) {
  ...
  if (!slug) {
    return (
      <div className="min-h-screen flex items-center justify-center text-slate-500">
        Invalid Dataset — missing parameters
      </div>
    );
  }
  ...
  const countryFormatted = countryRaw
    .split("-")
